...
function getAllUsers($pdo) {
    try {
        ...
        if ($users) {
            return sendResponse(['message' => 'Users retrieved successfully', 'users' => $users]);
        } else {
            return sendResponse(['message' => 'No users found'], 404);
        }
    } catch (PDOException $e) {
        return sendResponse(['message' => 'Error: ' . $e->getMessage()], 500);
    }
...
function updateUser($pdo, $input){
	$id = $input['id'] ?? '';
    if (empty($id)) {
        return sendResponse(['message' => 'Invalid input. required: user id'], 400);
    }
    ...
    if (!$routeExists) {
        return sendResponse(['message' => 'User not found'], 404);
    }
    ...
    try {
        ...
        if ($success) {
            return sendResponse(['message' => 'User updated successfully']);
        } else {
            return sendResponse(['message' => 'Failed to update user'], 500);
        }
    } catch (PDOException $e) {
        // Print SQL query and detailed error message for debugging
        return sendResponse(['message' => 'Failed to update user. Error: ' . $e->getMessage()], 500);
    }
}
